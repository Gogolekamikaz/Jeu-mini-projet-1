...
public class SuperPacmanAutoGeneratedMazeBehavior extends SuperPacmanBehavior {
    ...
    private boolean symmetricMaze = true;
     ...
     * @param width (int): behavior's width
     * @param height  (int): behavior's height
     */
    public SuperPacmanAutoGeneratedMazeBehavior(Window window, int width, int height){
        ...
        if(symmetricMaze){
            generateSymmetricMazeFeatures(width, height);
            createSymmetricMaze(width, height);
        } else {
    ...
    private void createMaze(int width, int height){
        ...
        //put random objects in dead ends
        for (int x = 0; x < width; x++) {
            for (int y = 0; y < height; y++) {
                if (((SuperPacmanCell)getCell(x, y)).getType() == SuperPacmanCellType.FREE_WITH_DIAMOND && wallNeigbours(x, y)) {
                    ((SuperPacmanCell) getCell(x, y)).setType(randomItem());
                }
            }
        }
     ...
     * @param height (int): behavior's height
     */
    private void generateSymmetricMazeFeatures(int width, int height) {

        //Initialize the outer walls
        for (int x = 0; x < Math.ceil(width/2.)+1; x++) {
            ((SuperPacmanCell)getCell(x, height-1)).setType(SuperPacmanBehavior.SuperPacmanCellType.WALL);
        }
        ...
        //Initialize Spawn Point
        spawnPoint = possibleSpwanPoint[0];
        ((SuperPacmanCell)getCell(spawnPoint.x, spawnPoint.y)).setType(SuperPacmanBehavior.SuperPacmanCellType.FREE_EMPTY);
    }
     ...
     * @param height (int): behavior's height
     */
    private void createSymmetricMaze(int width, int height){
        ...
        do {
            ...
            do{
                ...
                if(nextCell.x<(int)Math.ceil(width/2.) && nextCell.y>height/2-1 && ((SuperPacmanCell)getCell(nextCell.x, nextCell.y)).getType() == SuperPacmanCellType.NONE && neighboursCheck(nextCell, nextOrientation)){
                    lastMoves.add(nextCell);
                    ((SuperPacmanCell)getCell(nextCell.x, nextCell.y)).setType(SuperPacmanCellType.FREE_WITH_DIAMOND);
                    newCellFound = true;
                } else {
        ...
        for (int x = 0; x < (int)Math.ceil(width/2.); x++){
            for (int y = 0; y < height/2; y++) {
                ((SuperPacmanCell)getCell(x, y)).setType(((SuperPacmanCell)getCell(x, height-y-1)).getType());
            }
        }

        for (int x = (int)Math.ceil(width/2.); x < width; x++){
            for (int y = 0; y < height; y++) {
                ((SuperPacmanCell)getCell(x, y)).setType(((SuperPacmanCell)getCell(width-1-x, y)).getType());
            }
        }
        ...
        for(int i = 4; i<6; i++){
            ((SuperPacmanCell)getCell(i, 0)).setType(SuperPacmanBehavior.SuperPacmanCellType.FREE_EMPTY);
            ((SuperPacmanCell)getCell(i, 1)).setType(SuperPacmanBehavior.SuperPacmanCellType.FREE_EMPTY);
            ((SuperPacmanCell)getCell(i, 2)).setType(SuperPacmanBehavior.SuperPacmanCellType.FREE_EMPTY);
        }
    ...
    /**
     * @param orientations
     * @return random orientation (Orientation)
     */
     ...
     * @param coordinates
     * @param orientation
     * @return either a cell can be set as a path, regarding it's neighbours
     */
     ...
     * @param x
     * @param y
     * @return either a cell has 3 wall as neighbours
     */
    private boolean wallNeigbours(int x, int y){

        int wallNeigbours = 0;

        if (((SuperPacmanCell)getCell(x+1, y)).getType() == SuperPacmanCellType.WALL){
            wallNeigbours++;
        }
        if (((SuperPacmanCell)getCell(x-1, y)).getType() == SuperPacmanCellType.WALL){
            wallNeigbours++;
        }
        if (((SuperPacmanCell)getCell(x, y+1)).getType() == SuperPacmanCellType.WALL){
            wallNeigbours++;
        }
        if (((SuperPacmanCell)getCell(x, y-1)).getType() == SuperPacmanCellType.WALL){
            wallNeigbours++;
